...
const addStory = asyncHandler(async (req, res) => {
    try {
        const currentUser = await User.Id(req.user.userId);
        ...
        if (req.files || req.body.shareWith || req.body.location || req.body.tags) {
          if (req.files) {
            ...
            console.log('Response',cloudinaryResponses);
            const cloudinaryUrls = cloudinaryResponses.map(response => response.secure_url);
            console.log('ResponseUrl',cloudinaryUrls);
            storyData.files.push(...cloudinaryUrls);
          }
